...
void insert(Vector *vector, size_t index, void *value) {
	if (index<=vector->size) {
		if (vector->size>=vector->capacity)
				reserve(vector, vector->capacity * 2);
		char* prev = (char*)vector->data + (index * vector->element_size);
		char* next = (char*)vector->data + ((index + 1) * vector->element_size);
		memmove(next, prev, (vector->size - index) * vector->element_size);
		memcpy(prev, value, vector->element_size);
		vector->size++;
	}
}
...
void erase(Vector *vector, size_t index) {
	if (index<vector->size) {
		char* prev = (char*)vector->data + (index * vector->element_size);
		char* next = (char*)vector->data + ((index + 1) * vector->element_size);
		memmove(prev, next, (vector->size - (index + 1)) * vector->element_size);
		vector->size--;
	}
}
...
void erase_value(Vector *vector, void *value, cmp_ptr cmp) {
	size_t i = 0;
    while(i<vector->size) {
		void *temp = (char *)vector->data + (i * vector->element_size);
        if (!cmp(value, temp))
            erase(vector, i);
        else
            i++;
...
int person_cmp(const void *p1, const void *p2) {
	const Person *person_a = (const Person*)p1;
	const Person *person_b = (const Person*)p2;
	if (person_a->age != person_b->age)
		return person_b->age - person_a->age;
	else {
